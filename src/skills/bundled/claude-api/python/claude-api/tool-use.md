...
For connecting to MCP servers, see the MCP connector. For building your own MCP client, see [modelcontextprotocol.io](https://modelcontextprotocol.io/docs/develop/build-client).
...
When you use `tools`, we also automatically include a special system prompt for the model which enables tool use. The number of tool use tokens required for each model are listed below (excluding the additional tokens listed above). Note that the table assumes at least 1 tool is provided.
...
- **[Understand the concepts](/docs/en/agents-and-tools/tool-use/how-tool-use-works)** - Where tools run, how the loop works, and when to use tools.
- **[Build step by step](/docs/en/agents-and-tools/tool-use/build-a-tool-using-agent)** - The tutorial: from a single tool call to production.
- **[Browse all tools](/docs/en/agents-and-tools/tool-use/tool-reference)** - Directory of Anthropic-provided tools and properties.
